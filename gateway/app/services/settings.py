...
def load_settings() -> Settings:
    ...
    GATEWAY_ROLE = os.getenv("GATEWAY_ROLE", "both").lower()
    ...
    OUTBOX_STRIP_INTERNAL = os.getenv("OUTBOX_STRIP_INTERNAL", "").lower()
    ...
    CHANNEL_ROUTING_STRATEGY = os.getenv("CHANNEL_ROUTING_STRATEGY", "none").lower()
    SNOWFLAKE_ENABLED = os.getenv("SNOWFLAKE_ENABLED", "1").lower() in ("1", "true", "yes", "on")
    SNOWFLAKE_WORKER_ID = int(os.getenv("SNOWFLAKE_WORKER_ID", "0"))
    SNOWFLAKE_EPOCH_MS = int(os.getenv("SNOWFLAKE_EPOCH_MS", "1704067200000"))
    ...
    if CHANNEL_ROUTING_STRATEGY not in ("none", "channel_id"):
        CHANNEL_ROUTING_STRATEGY = "none"
    SNOWFLAKE_WORKER_ID = max(0, min(1023, SNOWFLAKE_WORKER_ID))
    if SNOWFLAKE_EPOCH_MS <= 0:
        SNOWFLAKE_EPOCH_MS = 1704067200000
    if OUTBOX_STRIP_INTERNAL in ("1", "true", "yes", "on"):
        OUTBOX_STRIP_INTERNAL = True
    elif OUTBOX_STRIP_INTERNAL in ("0", "false", "no", "off"):
        OUTBOX_STRIP_INTERNAL = False
    else:
        OUTBOX_STRIP_INTERNAL = GATEWAY_ROLE == "read"
    ROLE_READ = GATEWAY_ROLE in ("read", "both")
    ROLE_WRITE = GATEWAY_ROLE in ("write", "both")
    ...
    return Settings(
        ...
        OUTBOX_STRIP_INTERNAL=OUTBOX_STRIP_INTERNAL,
        ...
        CHANNEL_ROUTING_STRATEGY=CHANNEL_ROUTING_STRATEGY,
